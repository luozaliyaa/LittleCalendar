...
namespace LittleCalendar
{
    ...
    public sealed class DeadlineFields : StackPanel
    {
        ...
        private readonly ComboBox unit = new ComboBox { ItemsSource = new[] { "小时", "天（按 24 小时估算）", "工作日（人工指定截止）", "明确截止时间（人工指定）" } };
        ...
        private readonly TextBlock manualHint = UI.Text("", 12, "#A27536");
        ...
        public DeadlineFields(DeadlineSpec existing, DateTime selected)
        {
            ...
            unit.SelectedIndex = existing == null || existing.Unit == "hours" ? 0 : existing.Unit == "days" ? 1 : existing.Unit == "workingDays" ? 2 : 3;
            confirmed.IsChecked = existing == null || existing.Confirmed;
            original.Text = existing == null ? "" : existing.OriginalText;
            if (existing != null && (existing.Unit == "workingDays" || existing.Unit == "absolute")) {
            ...
            working = UI.Stack(manualHint, Row(UI.Field("人工核实的截止日期", endDate), UI.Field("人工核实的截止时间", endTime)));
        ...
        public DeadlineSpec Read()
        {
            ...
            var spec = new DeadlineSpec {
                ...
                Unit = unit.SelectedIndex == 0 ? "hours" : unit.SelectedIndex == 1 ? "days" : unit.SelectedIndex == 2 ? "workingDays" : "absolute",
        ...
        private void UpdatePreview()
        {
            ...
            manualHint.Text = unit.SelectedIndex == 2
                ? "工作日受节假日、调休及招聘方规则影响，不自动推算。请核实并填写截止时间（与起算时间使用同一时区）。"
                : "邮件已给出明确截止时刻。修改时请同时核对日期、时间和时区，再勾选“已核实截止时间”。";
